...
int board_late_init(void)
{
	...
	cmdline_handle();
#ifdef CONFIG_AMP
	if (gd->console_evt == 0x20) /* spacebar pressed ? */
		cli_loop();
...
char *board_fdt_chosen_bootargs(void *fdt)
{
	...
	if (!strcmp("mmc", env_get("devtype")) &&
		!strcmp("0", env_get("devnum")))
		env_update("bootargs", "root=/dev/mmcblk0p6");
	else if (!strcmp("mmc", env_get("devtype")) &&
		!strcmp("1", env_get("devnum")))
		env_update("bootargs", "root=/dev/mmcblk1p6");
