...
{
	struct udevice *dev;
	struct udevice *cursor;
	u32 ret = 0;
	int gpio,usb_net_node;

#if defined(CONFIG_ROCKCHIP_DMC_FSP)
	...
	ret = uclass_get_device_by_name(UCLASS_PINCTRL, "pinctrl", &cursor);
	if (ret) {
		printf("%s: Cannot get pinctrl: ret=%d\n", __func__, ret);
		return 0;
	}
	pinctrl_select_state(cursor, "default");

	gpio_request(GPIO0_D4, "som_led2");
	gpio_direction_output(GPIO0_D4, 1);

	/*
	 * The pinctrl of usb-net-reset gpio is referenced by /pinctrl/pinctrl-0.
	 * The state of all pins refered by /pinctrl/pinctrl-0 has been
	 * set to default in som led on commit.
	 */
	usb_net_node = fdt_node_offset_by_compatible(gd->fdt_blob, 0, "usb-net-reset");
	if (!usb_net_node) {
		printf("%s: Cannot find node: ret=%d\n", __func__, usb_net_node);
		return 0;
	}

	gpio = fdtdec_get_int(gd->fdt_blob,usb_net_node,"gpio",-1);
	if (gpio >= 0) {
		gpio_request(gpio, "usb_net_reset");
